...
## 6. What's in the bundle (`QuickAudit-System-v1.zip`)

```
QuickAudit-System-v1/
├── 00_README_Overview.md                    ← this document
├── 01_skill/
│   └── quick-audit/
│       └── SKILL.md                          ← scheduled task entry point
├── 02_design_docs/
│   ├── QuickAudit-Agent-Definition_v2.md     ← 16-step workflow, qualification rubric, voice rules
│   ├── QuickAudit-Output-Template_v2.md      ← audit body structure
│   ├── QuickAudit-Grading-Approach_v1.md     ← anchored-reasoning grading + calibration anchors
│   └── QuickAudit-Automation-Plan_v1.md      ← architecture, sheet schema, intake-zap contract
├── 03_code/
│   ├── render_audit_pdf.py                   ← generalized PDF renderer (JSON-driven)
│   └── build_audit_spec_example.py           ← worked example: how to build a spec JSON
├── 04_reference/
│   └── Apparel_Junction_Quick_Audit.pdf      ← Dennis's published reference, the canonical format
└── 05_sample_outputs/
    ├── ApparelJunction_Quick-Audit_v1.md      ← our markdown replication (calibration anchor)
    ├── ApparelJunction_Grading-Notes_v1.md
    ├── ExpertServicesUtah_Quick-Audit_v1.md   ← cold-submission test
    ├── ExpertServicesUtah_Grading-Notes_v1.md
    └── ExpertServicesUtah_Quick-Audit_v1.pdf  ← styled PDF sample
```
