...
The official Google Gmail MCP should still be used for drafts, and the official
Google Drive MCP should still be used for audit report file creation/uploads.
"""
...
def _credentials() -> Credentials:
    """Load or create local user OAuth credentials.

    Expected files:
    - GOOGLE_OAUTH_CLIENT_SECRETS_FILE: OAuth Desktop App client JSON
    - GOOGLE_OAUTH_TOKEN_FILE: local token cache path
    """

    token_path = Path(os.getenv("GOOGLE_OAUTH_TOKEN_FILE", "./secrets/google-oauth-token.json"))
    client_secret_path = Path(
        os.getenv("GOOGLE_OAUTH_CLIENT_SECRETS_FILE", "./secrets/google-oauth-client.json")
    )
    ...
    if creds and creds.expired and creds.refresh_token:
        ...
        if not client_secret_path.exists():
            raise FileNotFoundError(
                ...
                "Create an OAuth Desktop App client in Google Cloud Console and save it there."
            )
        flow = InstalledAppFlow.from_client_secrets_file(str(client_secret_path), GOOGLE_OAUTH_SCOPES)
...
def health_check() -> dict[str, Any]:
    ...
    return {
        "google_oauth_client_secrets_file_exists": Path(
            os.getenv("GOOGLE_OAUTH_CLIENT_SECRETS_FILE", "./secrets/google-oauth-client.json")
        ).exists(),
        "google_oauth_token_file_exists": Path(
            os.getenv("GOOGLE_OAUTH_TOKEN_FILE", "./secrets/google-oauth-token.json")
        ).exists(),
